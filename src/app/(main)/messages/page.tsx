...
export default function MessagesPage() {
  ...
  const isValidChat = useCallback((chat: Chat) => {
    ...
    return (
      chat && chat._id && flatParticipants.some((p) => p && p._id && p.username)
    );
  }, []);
  ...
  const getUserMessage = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${BASEURL}/api/get/user/chat/${userId}`,
        {
          headers: {
      ...
      const validChats = Array.isArray(chatData)
        ? chatData.filter((chat) => isValidChat(chat))
        : [];
      setChats(validChats);
          ...
          <>
                ...
                <div className="absolute inset-0 bg-opacity-20 z-50 flex items-center justify-center">
                  <div className="bg-white rounded-lg p-6 text-center">
                    ...
                    <div className="text-sm text-gray-600">
                      Hỗ trợ tất cả loại file, tối đa 10MB
                    </div>
                  </div>
                </div>
              )}
              {selectedChat.chatContext &&
              selectedChat.chatContext.length > 0 ? (
                <>
                  {selectedChat.chatContext.map((message, index) => {
                    const userId = localStorage.getItem("userId");
                    const flatParticipants = flattenParticipants(
                    ...
                    if (selectedChat.isGroupChat === true) {
                      ...
                      const otherUser = flatParticipants.find(
                        (p) => p._id !== userId
                      );
                    ...
                    const messageSender = flatParticipants.find(
                      (p) => p._id === message.userid
                    );
              ...
              <div className="p-2 sm:p-4 bg-gray-100 border-t">
                ...
                <div className="flex flex-wrap gap-2">
                  {attachedFiles.map((attachedFile, index) => (
                    <div
                      key={index}
                      className="relative bg-white rounded-lg p-2 border flex items-center gap-2 max-w-full sm:max-w-xs"
                    >
            ...
            {chatBlocked ? (
              <div className="p-2 sm:p-4 border-t bg-gray-100 text-center text-gray-500 font-semibold text-sm sm:text-base">
                Không thể kết nối tới người này. Người dùng đã tắt chức năng
                nhận tin nhắn.
              </div>
            ) : (
              <form
      ...
      const fetchUserMessage = async () => {
        try {
          ...
          if (selectedChat) {
            ...
            if (
              updatedSelectedChat &&
              JSON.stringify(updatedSelectedChat.chatContext) !==
                JSON.stringify(selectedChat.chatContext)
            ) {
              setSelectedChat(updatedSelectedChat);
            }
          }
    ...
    const handler = (message: WebSocketMessage) => {
      ...
      if (message.type === "online_friend") {
        if (
          ...
          typeof message.friends[0] === "object"
        ) {
  ...
  const handleSelectOnlineFriend = (friendId: string) => {
    const existingChat = chats.find((chat) => {
      const flatParticipants = flattenParticipants(chat.participants);
      return (
        !chat.isGroupChat && flatParticipants.some((p) => p._id === friendId)
      );
    });
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden">
              {isLoading ? (
                  ...
                  <div className="p-2">
                    {chats
                      ...
                      .map((chat) => {
                        ...
                        const lastMessage =
                          chat.lasttext ||
                          chat.chatContext?.[chat.chatContext.length - 1];
                      ...
                      >
                        <div className="relative">
                          <Image
                            ...
                            height={480}
                            src={
                              friend.avatar_link && friend.avatar_link !== ""
                                ? friend.avatar_link
                                : "/schoolimg.jpg"
                            }
                            alt={friend.username}
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-400 p-4">
                  ...
                  <div className="text-center">
                    <div className="font-medium mb-1">
                      Chưa có bạn bè hoạt động
                    </div>
                    <div className="text-sm">
                      Bạn bè sẽ xuất hiện khi online
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-500 text-sm">
                      {selectedChat.isGroupChat
                        ? `Nhóm có ${
                            flattenParticipants(selectedChat.participants)
                              .length
                          } thành viên`
                      ...
                      <>
                        ...
                        {showMembers && (
                          <div className="bg-white rounded-lg border p-3 max-h-48 overflow-y-auto">
                            ...
                            {flattenParticipants(selectedChat.participants).map(
                              (user) => (
                                <div
                                  key={user._id}
                              ...
                              {userData.friends
                                ...
                                .map((friend) => (
                                  <div
                                    ...
                                    onClick={() =>
                                      setSelectedFriendToAdd(friend._id)
                                    }
                                  >
                                    <Image
                                      width={32}
                                      height={32}
                                      src={
                                        friend.avatar_link || "/schoolimg.jpg"
                                      }
                                      alt={friend.username}
